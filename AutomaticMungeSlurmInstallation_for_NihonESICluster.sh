...
# 	cp AutomaticMungeSlurmInstallation_for_NihonESICluster.sh ~/
# 3) launch this script
#	cd ~/
# chmod -R 777 ./AutomaticMungeSlurmInstallation_for_NihonESICluster.sh
#	./AutomaticMungeSlurmInstallation_for_NihonESICluster.sh
...
# tokclu001-mstr1     48:df:37:56:51:40   192.168.246.11

# tokclu001-n001 	  20:67:7C:0C:BB:70   192.168.246.101
# tokclu001-n002      20:67:7C:0C:BB:60   192.168.246.102
# tokclu001-n003      20:67:7C:0C:B3:70   192.168.246.103
# tokclu001-n004      20:67:7C:0C:BA:10   192.168.246.104
# tokclu001-n005      20:67:7C:0C:BA:E0   192.168.246.105
# tokclu001-n006      20:67:7C:0C:B9:D0   192.168.246.106
# tokclu001-n007      20:67:7C:0C:BB:E0   192.168.246.107
# tokclu001-n008      20:67:7C:0C:B7:00   192.168.246.108
# tokclu001-n009      20:67:7C:0C:BA:50   192.168.246.109
# tokclu001-n010      20:67:7C:0C:BB:00   192.168.246.110
# tokclu001-n011      20:67:7C:0D:F0:80   192.168.246.111
# tokclu001-n012      20:67:7C:0D:EE:A0   192.168.246.112
# tokclu001-n013      20:67:7C:0D:EA:E0   192.168.246.113
# tokclu001-n014      20:67:7C:0D:EA:E0   192.168.246.114
# tokclu001-n015      20:67:7C:0C:BB:10   192.168.246.115
# tokclu001-n016      20:67:7C:0C:BB:A0   192.168.246.116

# 5) in case you want to launch the script with different IPs numbers: 
# suppose you have 3 computes nodes with IP 192.168.70.121, 192.168.70.122, 192.168.70.123
# then you need to: 
# 	- replace all "192.168.246" with "192.168.70"
#	- replace all "101..116" with "121..123"
...
for i in {101..116}; do
  ssh root@192.168.246.$i yum remove mariadb-server mariadb-devel -y
  ssh root@192.168.246.$i yum remove slurm munge munge-libs munge-devel -y
  ssh root@192.168.246.$i systemctl stop munge
  ssh root@192.168.246.$i systemctl stop slurmd
  ssh root@192.168.246.$i systemctl stop slurmctld
  ssh root@192.168.246.$i userdel -r slurm
  ssh root@192.168.246.$i userdel -r munge
done
...
for i in {101..116}; do
  ssh root@192.168.246.$i systemctl stop firewalld
  ssh root@192.168.246.$i systemctl disable firewalld
done
...
for i in {101..116}; do
  ssh root@192.168.246.$i sed -i 's/SELINUX=.*/SELINUX=disabled/g' /etc/selinux/config
done
...
then
	...
	# compute nodes
	for i in {101..116}; do	
	  ssh root@192.168.246.$i yum -y update
...
for i in {101..116}; do 
  # setup local scratch
  ssh root@192.168.246.$i mkdir /scratch
  ssh root@192.168.246.$i chmod 777 /scratch
  ssh root@192.168.246.$i chmod +t /scratch
done
...
for i in {101..116}; do
  scp ~/tmp.sh root@192.168.246.$i:~/
  ssh root@192.168.246.$i bash ~/tmp.sh
  # checking 
  ssh root@192.168.246.$i grep '1127' /etc/passwd
  ssh root@192.168.246.$i grep '1128' /etc/passwd
done
...
for i in {101..116}; do
  ssh root@192.168.246.$i yum install epel-release -y
  ssh root@192.168.246.$i yum install munge munge-libs munge-devel -y
  ssh root@192.168.246.$i rm -rf /etc/munge/munge.key # removing old key
  scp /etc/munge/munge.key root@192.168.246.$i:/etc/munge
...
for i in {101..116}; do
  ssh root@192.168.246.$i yum install rpm-build gcc openssl openssl-devel libssh2-devel pam-devel numactl numactl-devel hwloc hwloc-devel lua lua-devel readline-devel rrdtool-devel ncurses-devel gtk2-devel man2html libibmad libibumad perl-Switch perl-ExtUtils-MakeMaker -y
...
for i in {101..116}; do
  ssh root@192.168.246.$i yum install mariadb-server mariadb-devel -y
...
for i in {101..116}; do
  scp -r ~/slurm_rpms root@192.168.246.$i:~/
done
echo "Slurm RPMS transfer to compute nodes completed."

cd ~/slurm_rpms
yum install mailx -y
yum install -y slurm-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-perlapi-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-contribs-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-devel-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-example-configs-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-libpmi-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-openlava-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-pam_slurm-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-slurmctld-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-slurmd-17.11.7-1.el7.x86_64.rpm  # installing slurmd also on the master since in our minicluster setting the master node can act like debug compute node
yum install -y slurm-slurmdbd-17.11.7-1.el7.x86_64.rpm
yum install -y slurm-torque-17.11.7-1.el7.x86_64.rpm
...
# similar as master node but we don't install slurm database and slurm controller daemon 		
  ssh root@192.168.246.$i yum install mailx -y
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-perlapi-17.11.7-1.el7.x86_64.rpm  
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-contribs-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-devel-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-example-configs-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-libpmi-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-openlava-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-pam_slurm-17.11.7-1.el7.x86_64.rpm
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-slurmd-17.11.7-1.el7.x86_64.rpm  
  ssh root@192.168.246.$i yum install -y slurm_rpms/slurm-torque-17.11.7-1.el7.x86_64.rpm
...
# 
ControlMachine=tokclu001-mstr1 
ControlAddr=192.168.246.11
#
...
# 
# COMPUTE NODES
NodeName=tokclu001-n001 NodeAddr=192.168.246.101 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n002 NodeAddr=192.168.246.102 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n003 NodeAddr=192.168.246.103 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n004 NodeAddr=192.168.246.104 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n005 NodeAddr=192.168.246.105 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n006 NodeAddr=192.168.246.106 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n007 NodeAddr=192.168.246.107 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n008 NodeAddr=192.168.246.108 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n009 NodeAddr=192.168.246.109 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n010 NodeAddr=192.168.246.110 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n011 NodeAddr=192.168.246.111 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n012 NodeAddr=192.168.246.112 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n013 NodeAddr=192.168.246.113 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n014 NodeAddr=192.168.246.114 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n015 NodeAddr=192.168.246.115 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-n016 NodeAddr=192.168.246.116 CPUs=32 Sockets=2 CoresPerSocket=16 ThreadsPerCore=1 State=UNKNOWN
NodeName=tokclu001-mstr1 NodeAddr=192.168.246.11 CPUs=12 Sockets=2 CoresPerSocket=6 ThreadsPerCore=1 State=UNKNOWN
PartitionName=production Nodes=tokclu001-n[001-016] Default=YES MaxTime=INFINITE State=UP
PartitionName=debug Nodes=tokclu001-mstr1 Default=YES MaxTime=INFINITE State=UP
EOF
...
for i in {101..116}; do
  scp /etc/slurm/slurm.conf root@192.168.246.$i:/etc/slurm
...
for i in {101..116}; do
  scp /etc/slurm/cgroup.conf root@192.168.246.$i:/etc/slurm
...
for i in {101..116}; do
  ssh root@192.168.246.$i mkdir /var/run/slurm
  ssh root@192.168.246.$i chown slurm: /var/run/slurm
  ssh root@192.168.246.$i chmod 755 /var/run/slurm
  ssh root@192.168.246.$i mkdir /var/log/slurm/
  ssh root@192.168.246.$i chown slurm: /var/log/slurm/
  ssh root@192.168.246.$i chmod 755 /var/log/slurm/
  ssh root@192.168.246.$i mkdir /var/spool/slurm
  ssh root@192.168.246.$i chown slurm: /var/spool/slurm
  ssh root@192.168.246.$i chmod 755 /var/spool/slurm
done
...
for i in {101..116}; do
  ...
  ssh root@192.168.246.$i sed -i -e 's@PIDFile=.*@PIDFile=replacement=@g' /usr/lib/systemd/system/slurmd.service
done
...
# on COMPUTE node : 
for i in {101..116}; do
  ssh root@192.168.246.$i systemctl enable slurmd.service
  ssh root@192.168.246.$i systemctl start slurmd.service
  ssh root@192.168.246.$i systemctl status slurmd.service
